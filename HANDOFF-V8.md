...
| 3 | Skeleton loading + designed empty states on every list | **PART DONE — finish this before Unit 4.** `components/ui/skeleton.tsx` is the ONE skeleton vocabulary (no `"use client"`, so `loading.tsx` can import it) — compose it, do not write a third. **Done:** 41 files now use `EmptyState`; `loading.tsx` exists for `(app)` root, `projects/[id]`, and all three `/finance/*` screens. **Remaining:** `loading.tsx` for the other list routes (leads · quotations · projects list · procurement · inventory · vendors · rfq · orders · hr · reports), and 7 files still hand-roll a `border-dashed` empty state — `dashboard/workspace-ui.tsx` · `leads/insights/insights-view.tsx` · `projects/[id]/finance/schedule-editor.tsx` · `projects/[id]/project-workspace.tsx` · `projects/[id]/report/report-builder.tsx` · `quotations/quote-builder.tsx` · `vendors/[id]/page.tsx` (check each — some may be drop zones, not empty states). |
